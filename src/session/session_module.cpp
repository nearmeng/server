...
BOOL CSessionModule::init(BOOL bResume)
{
    ...
    nRetCode = m_ClientSessionPool.init(sstdServerSessionDefault, g_ServerConfig.nInitDefaultClientSessionCount, bResume);
...
BOOL CSessionModule::TRAVERSE_SESSION::operator()(uint64_t qwSessionID, CLIENT_SESSION* pSession)
{
    ...
    if (dwCurrTime > pSession->dwLastPingTime + g_ServerConfig.nClientPingTimeout / 1000)
    {
