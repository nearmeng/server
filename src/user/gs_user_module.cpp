...
#include "db_proto/user_db_data.pb.h"
#include "db_proto/role_db_data.pb.h"
...
BOOL CUserModule::init(BOOL bResume)
{
    ...
    nRetCode = m_UserPool.init(sstdUser, g_ServerConfig.nUserCount, bResume);
...
BOOL CUserModule::TRAVERSE_USER_SESSION_CHECK::operator()(uint64_t qwUserID, USER* pUser)
{
    ...
    if (pUser->qwSessionID == 0 && CTimeMgr::instance().get_time_sec() > pUser->nSessionStopTime + g_ServerConfig.nUserKickAfterSessionStopTime / 1000)
    {
...
BOOL USER::load_role_base(const char* pData, uint32_t dwSize)
{
    ...
    strxcpy(stRoleBase[nRoleCount].szRoleName, RoleBaseData.role_name().c_str(), MAX_USER_ROLE_NAME_LEN);
